...
- **No tiene Liquidador.** Se eliminó el botón del menú (llamaba a `abrirLiqADH()`, que nunca existió en este archivo — quedó roto desde hace tiempo). No reintroducir ese botón en ADH.
- **Pedido a Fábrica** (botón en el menú de escritorio): modal aislado del carrito de cotización para armar la orden de compra al proveedor. Busca ítem del catálogo (o texto libre) + cantidad → PDF agrupado por marca/proveedor, con la **TRM real de BanRep** (`trmBanRepPura`, sin el +15%) y valores en USD y COP. No guarda historial en SharePoint todavía — solo PDF descargable. Funciones: `abrirPedidoFabrica`, `agregarItemPedidoFab`, `generarPDFPedidoFabrica`, `cargarTRMBanRepPuraPF`.
  - **Costo por unidad de pedido:** `COSTO_AYJ` en SharePoint es por Kg/unidad base, pero se pide por presentación (BULTO 25 KG, GARRAFA, etc). `parseContenidoPresentacion(presentacion)` extrae el primer número del texto de Presentación (ej. "BULTO 25 KG" → 25) y `costoUnidadPedido = costoBaseAYJ × contenido`. Si la Presentación no trae número parseable, cae a contenido=1 (deja el costo tal cual, no revienta pero puede quedar subvaluado — si un producto muestra un costo raro en el PDF, revisar primero si su Presentación trae el número).
- **Botón "📊 MARGEN"** (junto al MG existente, NO es lo mismo): compara Precio de Venta del carrito (sin IVA, con descuento, a la TRM del cotizador) vs Costo de Fábrica (mismo `parseContenidoPresentacion`, a `trmBanRepPura`) → margen real en COP y %. Protegido con `CLAVE_MG` (misma clave que el MG de comisión, pero es un candado independiente: `window.margenDesbloqueado`). El botón "MG" (píldora verde) sigue siendo la herramienta de descuento extra/comisión — no se tocó, no confundir los dos.

### Fixes recientes
- **02-sep-2026 — Quitar Liquidador, arreglar color de botón, costo real por presentación, botón Margen Real.** Ver secciones arriba (Pedido a Fábrica, botón MARGEN).
